...
class Net(nn.Module):
    ...
    def expand(self, pre_cos, t):
        layers = len(pre_cos[0])
        layers_cos = torch.zeros(layers)

        for i in range(layers):
            temp = torch.sum(torch.sum(pre_cos[:, i], dim=0) / len(pre_cos))
            layers_cos[i] = temp / t

        _, layers_sort = torch.sort(layers_cos)
